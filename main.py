...
async def lifespan(app: FastAPI):
    ...
    if not os.path.exists(settings.DB_DIR):
        print("🚀 Beginning initialization of RAG system and Vector Store for the first time...")
        os.makedirs(settings.DATA_DIR, exist_ok=True)
        if os.path.exists(pdf_path):
            initialize_rag_system(pdf_path)
        else:
            print(f"⚠️ Critical Alert: Please place the PDF file in the path: {pdf_path}")
    else:
        print("Found existing Vector Store. Skipping RAG system initialization.")
    yield
