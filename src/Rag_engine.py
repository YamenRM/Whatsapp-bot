...
def initialize_rag_system(pdf_path: str):
    ...
    if not os.path.exists(pdf_path):
        raise FileNotFoundError(f"ملف الـبيانات  غير موجود في المسار: {pdf_path}")
        
    print("⏳ ... جاري تحميل ملف الـبيانات وتقسيمه")
    ...
    print("💾 جاري توليد الـ Embeddings وحفظها في ChromaDB...")
    vector_store = Chroma.from_documents(
        documents=chunks,
        embedding=embeddings,
        persist_directory=settings.DB_DIR
    )
    print("🎯 تمت العملية بنجاح! قاعدة البيانات جاهزة للاستخدام.")
    return vector_store
